...
# Note - each table you restore to should be empty.  Otherwise you will probably
# run into "duplicate key value violates unique constraint" which will stop the import
# for that table
...
pg_restore --clean --dbname=$PGDATABASE -h $PGHOST --verbose --format=c --jobs=1 --username $PGUSER --schema $SCHEMA --disable-triggers  < /app/data/import.dump > /app/data/import.log 2>&1
